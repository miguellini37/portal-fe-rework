...
const money = (n?: number) =>
  typeof n === 'number' && n > 0
    ...
    : '—';
const moneyPair = (a?: number, b?: number) => (
  (a = num(a)),
  (b = num(b)),
  a == null && b == null
    ? '—'
    : a != null && b != null
      ? a === b
        ? money(a)
        : `${money(Math.min(a, b))} - ${money(Math.max(a, b))}`
      : money(a ?? b)
);
      ...
      {two ? (
        <div className="range-group">
          <input
            ...
            placeholder="Min"
            value={min ?? ''}
            onChange={(e) => onChange(p(e.target.value), max)}
          />
          ...
          <input
            ...
            placeholder="Max"
            value={max ?? ''}
            onChange={(e) => onChange(min, p(e.target.value))}
          />
          <button
...
export const BenefitsTab: React.FC<Props> = ({ company, setCompany, editMode }) => {
  ...
  const b = useMemo<BenefitsPayload>(
    () => ({
      baseSalaryMin: num(base?.baseSalaryMin),
      baseSalaryMax: num(base?.baseSalaryMax),
      commissionMin: num(base?.commissionMin),
      commissionMax: num(base?.commissionMax),
      totalCompMin: num(base?.totalCompMin),
      totalCompMax: num(base?.totalCompMax),
  ...
  const usedIds = new Set(
    cats.map((c) => byTitle[(c.title ?? '').toLowerCase()]?.id).filter(Boolean) as string[]
  );

  const setBenefits = (
    patch: Partial<BenefitsPayload> | ((cur: BenefitsPayload) => Partial<BenefitsPayload>)
  ) =>
    setCompany((prev) => {
      const curRaw = (prev.benefits ?? {}) as Partial<BenefitsPayload>;
      const cur: BenefitsPayload = {
        baseSalaryMin: num(curRaw.baseSalaryMin),
        baseSalaryMax: num(curRaw.baseSalaryMax),
        commissionMin: num(curRaw.commissionMin),
        commissionMax: num(curRaw.commissionMax),
        totalCompMin: num(curRaw.totalCompMin),
        totalCompMax: num(curRaw.totalCompMax),
  ...
  const upCats = (f: (x: SpecificBenefits[]) => SpecificBenefits[]) =>
    setBenefits((cur) => ({ specificBenefits: f(cur.specificBenefits ?? []) }));
  const updCat = (i: number, f: (c: SpecificBenefits) => SpecificBenefits) =>
    upCats((cs) => cs.map((c, j) => (j === i ? f(c) : c)));
  const addCat = (presetId?: string) =>
    upCats((cs) => {
      ...
      return [
        ...cs,
        p
          ? { title: p.title, icon: p.icon, description: p.suggestions }
          : { title: 'New Category', icon: '', description: [] },
      ];
    });
  ...
  const renderCat = (c: SpecificBenefits, i: number) => {
    ...
    const opts = Array.from(
      new Set([...(preset?.suggestions ?? []), ...(c.description ?? [])])
    ...
    const onItems = (mv: MultiValue<Option>) =>
      upCats((cs) =>
        cs.map((c2, j) => (j === i ? { ...c2, description: mv.map((o) => o.value) } : c2))
      );
    return (
      <div key={`SpecificBenefits-${i}`} style={{ marginBottom: 16 }}>
          ...
          <>
            <div className="cat-header">
              <input
                ...
                onChange={(e) => updCat(i, (x) => ({ ...x, title: e.target.value }))}
              />
              <input
                type="text"
                value={c.icon ?? ''}
                ...
                onChange={(e) => updCat(i, (x) => ({ ...x, icon: e.target.value }))}
              />
              <button className="btn btn-secondary" type="button" onClick={() => removeCat(i)}>
                Remove
              </button>
            </div>
        ...
        {editMode ? (
          <CreatableSelect<Option, true>
            ...
            components={{
              MultiValueContainer: (p: any) => (
                <span className={`pill ${color(i)} pill-edit`}>{p.children}</span>
              ),
            }}
          ...
          <div className="pill-list">
            {(c.description ?? []).map((it, j) => (
              <span key={`${it}-${j}`} className={`pill ${color(i)}`}>
                {it}
              </span>
            ))}
          </div>
  ...
  return (
    <div
      ...
      onKeyDownCapture={(e) => editMode && e.stopPropagation()}
    >
      <section className="card values-card">
        <div className="comp-box">
          ...
          {editMode ? (
            ...
            FIELDS.map((f) => {
              const min = (b as BenefitsPayload)[`${f.k}Min`];
              const max = (b as BenefitsPayload)[`${f.k}Max`];
              return (
                <div key={f.k} className={`benefit-card ${f.cls}`}>
                  <span>{f.label}</span>
                  <strong style={{ color: '#000' }}>{moneyPair(min, max)}</strong>
                </div>
              );
            })
          )}
        ...
        {editMode && (
          <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
            <button className="btn btn-primary" type="button" onClick={() => addCat()}>
              + Add Category
            </button>
          </div>
        )}
      ...
      <section className="card env-card">
        <h3 className="section-title" style={{ marginTop: 0 }}>
          Student-Athlete Specific Benefits
        </h3>
        {editMode && (
          <p className="muted" style={{ marginBottom: 12 }}>
            Tip: Use presets, then add your own items (Enter). Icons accept emoji.
          </p>
        )}
