...
export const RecruitingTab: FC<Props> = ({ company, editMode, setCompany }) => {
  const r = useMemo<Required<IRecruitingPayload>>(() => ensureRecruiting(company.recruiting), [company.recruiting]);
...
const setR = useCallback(
  ...
  const addPreset = (p: RecruitingCategory) =>
    setR(curr => (curr.strategy.some(s => s.title === p.title) ? curr : { ...curr, strategy: [...curr.strategy, p] }));
  const addStrategy = () =>
    setR(curr => ({ ...curr, strategy: [...curr.strategy, { icon: '🧩', title: '', description: '' }] }));
  ...
  const updateStep = (i: number, value: string) => setR(curr => ({ ...curr, processSteps: setAt(curr.processSteps, i, value) }));
  const removeStep = (i: number) => setR(curr => ({ ...curr, processSteps: removeAt(curr.processSteps, i) }));
  ...
  return (
    <div className="recruiting-grid">
        ...
        {editMode && (
          <div className="preset-row">
            ...
            {STRATEGY_PRESETS.map(p => (
              <button key={p.title} className="preset-chip" type="button" onClick={() => addPreset(p)}>
                <span className="preset-chip-icon" aria-hidden="true">{p.icon}</span>{p.title}
              </button>
            ))}
            <button className="preset-chip" type="button" onClick={addStrategy}>+ Custom</button>
          </div>
        )}
        ...
        <ul className={`strategy-list${editMode ? ' strategy-list-1col' : ' strategy-list-2col'}`}>
          ...
          {r.strategy.map((s, i) => (
            <li key={i} className="strategy-item">
              {editMode ? (
                <>
                  <input className="strategy-icon-input" value={s.icon ?? ''} maxLength={2}
                    onChange={e => updateStrategy(i, { icon: e.target.value })} aria-label="Icon" />
                  <input className="strategy-title-input" value={s.title}
                    onChange={e => updateStrategy(i, { title: e.target.value })} aria-label="Title" />
                  <textarea className="strategy-desc-input" value={s.description ?? ''}
                    onChange={e => updateStrategy(i, { description: e.target.value })} rows={2} aria-label="Description" />
                  <button className="btn btn-tertiary" type="button" onClick={() => removeStrategy(i)}>Remove</button>
                </>
              ) : (
          ...
          <div className="process-editor">
            {r.processSteps.map((step, i) => (
              <div key={i} className="process-row">
                ...
                <input className="process-input" value={step}
                  onChange={e => updateStep(i, e.target.value)} aria-label={`Step ${i + 1}`} />
                <button className="btn btn-tertiary" type="button" onClick={() => removeStep(i)}>Remove</button>
              </div>
            ))}
            <button className="btn btn-secondary" type="button" onClick={addStep}>Add step</button>
          </div>
        ) : (
      ...
      <div className="card">
        ...
        {editMode && (
          <>
            <button className="btn btn-secondary" type="button" onClick={() => setShowAddRecruiter(true)}>
              + Add Recruiter
            </button>
            {showAddRecruiter && (
              <div className="recruiter-search-modal">
                <input className="recruiter-search-input" type="text" value={search}
                  onChange={e => setSearch(e.target.value)} placeholder="Search employees..." autoFocus />
                ...
                <button className="btn btn-tertiary" type="button" onClick={() => setShowAddRecruiter(false)}>
                  Cancel
                </button>
              </div>
            )}
